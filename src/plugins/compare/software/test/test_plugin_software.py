...
class TestComparePluginSoftware(ComparePluginTest):
    ...
    def test_get_intersection_of_software(self):
        self.fw_one.processed_analysis['software_components'] = {'summary': {'software a': self.fw_one.get_uid()}}
        self.fw_two.processed_analysis['software_components'] = {'summary': {'software a': self.fw_two.get_uid(), 'software b': self.fw_two.get_uid()}}
    ...
    def test_get_exclustive_software(self):
        ...
        self.assertIn('software a', result[self.fw_one.get_uid()], "fw_one not exclusive to one")
        self.assertNotIn('software a', result[self.fw_two.get_uid()], "fw_two in exclusive file of fw one")
        self.assertTrue(result['collapse'])

    def test_get_software_in_more_than_one_but_not_in_all(self):
        self.fw_one.processed_analysis['software_components'] = {'summary': {'software a': self.fw_one.get_uid()}}
        self.fw_two.processed_analysis['software_components'] = {'summary': {'software a': self.fw_two.get_uid()}}
        ...
        for i in range(len(fo_list)):
            tmp_result_dict['exclusive_software'][fo_list[i].get_uid()] = {}
        result = self.c_plugin._get_software_in_more_than_one_but_not_in_all(fo_list, tmp_result_dict)
        self.assertIsInstance(result, dict, "result is not a dict")
        self.assertIn('software a', result[self.fw_one.get_uid()], "foo not in result fw one")
        self.assertIn('software a', result[self.fw_two.get_uid()], "foo not in result fw_two")
        self.assertNotIn('software a', result[self.fw_three.get_uid()], "foo in result fw_three")
        self.assertTrue(result['collapse'])
