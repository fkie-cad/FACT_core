...
TEST_DATA_UPGRADE_RAW = b''
...
@pytest.mark.parametrize('input_data, fingerprint', [
    ...
    (b'RANDOM_DATA--=</Begin HP Signed File Fingerprint\\>=-- The Fingerprint --=</End HP Signed File Fingerprint\\>=--SOME_MORE_RANDOM_DATA', b'--=</Begin HP Signed File Fingerprint\\>=-- The Fingerprint --=</End HP Signed File Fingerprint\\>=--')
])
...
def test_extract_file_from_upgrade():
    ...
    expected_path_of_dumped_file = Path(tmp_dir.name, 'HP_Color_LaserJet_CP4525.bin')

    _extract_file_from_upgrade(TEST_DATA_UPGRADE_RAW, TEST_COMMAND, tmp_dir.name)
