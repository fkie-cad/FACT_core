...
def test_process_qemu_job():
    ...
    with mock_patch(qemu_exec, 'check_qemu_executability', lambda *_: test_results):
        ...
        assert results == {
            uid: {
                'path': 'test_path', 'results': {
                    'test_arch': test_results, 'test_arch_2': test_results
                }
            }
        }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({
            'arch': {
                'option': {
                    'stdout': 'foo', 'stderr': '', 'return_code': '0'
                }
            }
        }, True),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({
            'parameter': {
                'output': b64encode(b'').decode()
            }
        }, ''),
        ({
            'parameter': {
                'output': b64encode(b'foobar').decode()
            }
        }, 'foobar'),
        ({
            'parameter': {
