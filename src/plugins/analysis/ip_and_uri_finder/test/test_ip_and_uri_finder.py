...
class TestAnalysisPluginIpAndUriFinder:
    def test_process_object_ips(self, ip_and_uri_finder_plugin):
        ...
        assert results['uris'] == []
        assert set([('1.2.3.4', '47.913, -122.3042'), ('1.1.1.123', '-37.7, 145.1833')]) == set(results['ips_v4'])
        assert len([('1.2.3.4', '47.913, -122.3042'), ('1.1.1.123', '-37.7, 145.1833')]) == len(results['ips_v4'])
        assert set([('1234:1234:abcd:abcd:1234:1234:abcd:abcd', '2.1, 2.1'), ('2001:db8:0:0:8d3::', '3.1, 3.1')]), set(results['ips_v6'])
        assert len([('1234:1234:abcd:abcd:1234:1234:abcd:abcd', '2.1, 2.1'), ('2001:db8:0:0:8d3::', '3.1, 3.1')]), len(results['ips_v6'])
    ...
    def test_process_object_uris(self, ip_and_uri_finder_plugin):
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            processed_object = ip_and_uri_finder_plugin.process_object(tmp_fo)
            results = processed_object.processed_analysis[ip_and_uri_finder_plugin.NAME]
        assert set(['http://www.google.de', 'https://www.test.de/test/', 'ftp://ftp.is.co.za/rfc/rfc1808.txt', 'telnet://192.0.2.16:80/']), set(results['uris'])
    ...
    def test_add_geo_uri_to_ip(self, ip_and_uri_finder_plugin):
        ...
        assert 'http://www.google.de' == results['uris']
    ...
    def test_remove_blacklisted(self, ip_and_uri_finder_plugin):
        ...
        ip_and_uri_finder_plugin._remove_blacklisted(input_list, blacklist) == ['blah']
