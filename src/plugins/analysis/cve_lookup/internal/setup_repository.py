...
def exists(table_name: str) -> bool:
    return bool(list(DATABASE.select_query(QUERIES['sqlite_queries']['exist'].format(table_name))))


def drop_table(table_name: str):
    DATABASE.table_manager(QUERIES['sqlite_queries']['drop'].format(table_name))


def delete_outdated_feeds(delete_outdated_from: str, use_for_selection: str):
    DATABASE.table_manager(query=QUERIES['sqlite_queries']['delete_outdated'].format(delete_outdated_from, use_for_selection))


def extract_relevant_feeds(from_table: str, where_table: str) -> list:
    return list(DATABASE.select_query(query=QUERIES['sqlite_queries']['extract_relevant'].format(from_table, where_table)))


def insert_into(query: str, table_name: str, input_data: list):
    DATABASE.insert_rows(query=QUERIES['sqlite_queries'][query].format(table_name), input_t=input_data)


def create(query: str, table_name: str):
    DATABASE.table_manager(QUERIES['sqlite_queries'][query].format(table_name))
...
def get_years_from_database():
    return [el[0] for el in DATABASE.select_query(query=QUERIES['sqlite_queries']['get_years_from_cve'])]
