...
class TestAnalysisPluginInit:
    test_file_not_text = FileObject(file_path='{}etc/systemd/system/foobar'.format(_test_init_dir))
    ...
    def test_root_uid_is_none(self, analysis_plugin):
        ...
        assert processed_file.processed_analysis[analysis_plugin.PLUGIN_NAME]['summary'] != []
    ...
    def test_get_sysvinit_config(self, analysis_plugin):
        ...
        assert 'if [ true != "$INIT_D_SCRIPT_SOURCED" ] ; then\n    set "$0" "$@"; INIT_D_SCRIPT_SOURCED=true . /lib/init/init-d-script\nfi' in result['script'], 'record not found'
