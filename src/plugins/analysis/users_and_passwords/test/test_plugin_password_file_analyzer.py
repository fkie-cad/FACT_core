...
def test_crack_hash_failure():
    ...
    result_entry = {}
    assert crack_hash(b':'.join(passwd_entry[:2]), result_entry) is False  # pylint: disable=protected-access
...
def test_crack_hash_success():
    ...
    result_entry = {}
    assert crack_hash(passwd_entry.encode(), result_entry, '--format=dynamic_82') is True  # pylint: disable=protected-access
