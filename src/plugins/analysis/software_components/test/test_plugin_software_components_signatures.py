...
class TestSoftwareSignatureMeta(SignatureTestingMeta):
    def test_check_meta_fields(self):
        missing_fields = self.check_meta_fields(SIGNATURE_PATH)
        assert not missing_fields, f'Missing meta fields: {missing_fields.__str__()}'

    def test_check_meta_fields_missing(self):
        missing_fields = self.check_meta_fields(TEST_SIGNATURE_PATH)
        assert len(missing_fields) == 3  # noqa: PLR2004
        assert set(missing_fields) == {
            'ALL in missing_meta_2',
            'description in missing_meta_1',
            'website in missing_meta_1',
        }


class TestAllSoftwareSignaturesMatched:
    def setup_method(self):
        self.resulter = SignatureTestingMatching()

    def test_all_signatures_matched(self):
        diff = self.resulter.check(SIGNATURE_PATH, TEST_DATA_DIR / 'software_component_test_list.txt')
