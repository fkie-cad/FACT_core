...
class TestAnalysisPluginsSoftwareComponents:
    def test_process_object(self, analysis_plugin):
        ...
        len(results) == 2, 'incorrect number of software components found'
        ...
        results['MyTestRule']['meta']['website'] == 'http://www.fkie.fraunhofer.de', 'incorrect website from yara meta'
    ...
    def test_get_version_from_meta(self, analysis_plugin):
        version = 'v15.14.1a'
        assert analysis_plugin.get_version(f'Foo {version}', {'version_regex': 'v\\d\\d\\.\\d\\d\\.\\d[a-z]'}) == version, 'version not found correctly'
    ...
    def test_update_os_key(self, analysis_plugin):
        test_file = FileObject(file_path=os.path.join(TEST_DATA_DIR, 'yara_test_file'))
        test_file.processed_analysis[analysis_plugin.NAME] = dict(summary=['Linux Kernel'], tags={'OS': {'value': 'Fire OS'}})
