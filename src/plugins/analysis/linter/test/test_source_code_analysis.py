...
def test_process_object_not_supported(stub_plugin, test_object, monkeypatch):
    monkeypatch.setattr('storage.globals._fsorganizer.generate_path_from_uid', lambda _: test_object.file_path)
...
def test_process_object_this_file(stub_plugin, monkeypatch):
    ...
    monkeypatch.setattr("storage.globals._fsorganizer.generate_path_from_uid", lambda _: test_file.file_path)
