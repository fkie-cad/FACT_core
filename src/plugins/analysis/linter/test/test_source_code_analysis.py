...
class TestSourceCodeAnalysis:
    def test_process_object_not_supported(self, analysis_plugin, test_object, monkeypatch):
        ...
        assert result.processed_analysis[analysis_plugin.NAME] == {'summary': [], 'warning': 'Is not a script or language could not be detected'}
    ...
    def test_process_object_no_issues(self, analysis_plugin, test_object, monkeypatch):
        ...
        monkeypatch.setattr('storage.fsorganizer.FSOrganizer.generate_path_from_uid', lambda _self, _: test_object.file_path)
        monkeypatch.setattr('plugins.analysis.linter.code.source_code_analysis.linters.run_pylint', lambda self, file_path: [])
