...
class AnalysisPlugin(AnalysisBasePlugin):
    ...
    def process_object(self, file_object):
        ...
        if script_type is None:
            analysis['result'] = {'warning': 'Is not a script or language could not be detected'}
            return file_object
        ...
        if script_type not in self.linter_impls:
            ...
            analysis['result'] = {'warning': f'Unsupported script type: {script_type}'}
            return file_object

        issues = self.linter_impls[script_type](file_object.file_path)

        if len(issues) > 0:
            analysis['result'] = {'full': sorted(issues, key=lambda k: k['symbol'])}
            analysis['summary'] = [f'Warnings in {script_type} script']
        return file_object
    ...
    def _get_script_type(self, file_object):
        ...
        file_object.processed_analysis['file_type']['result']['linguist'] = ''.join([f'{k:<10} {str(v):<10}\n' for k, v in output_json[container_path].items()])
