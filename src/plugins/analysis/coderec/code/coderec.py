...
class AnalysisPlugin(AnalysisPluginV0):
    ...
    def __init__(self):
        metadata = AnalysisPluginV0.MetaData(
            ...
            mime_whitelist=['application/octet-stream', 'linux/kernel'],
...
def _add_region_colors(regions: list[Region], output_svg: bytes) -> None:
    ...
    type_to_color = {type_: color for type_, color in zip(type_list, color_list, strict=True) if type_ in types}
...
def _run_coderec_in_docker(file: FileIO) -> tuple[str, bytes]:
    with TemporaryDirectory() as tmp_dir:
        result = run_docker_container(
            DOCKER_IMAGE,
            command='--big-file /io/input -F svg -O /io/regions_plot.svg --no-title --si-labels',
            mounts=[
