...
from sqlalchemy import Column, ColumnElement, Integer, Select, cast, column, func, or_, select
...
class FrontEndDbInterface(DbInterfaceCommon):
    ...
    @staticmethod
    def _query_all_plugins_of_object(query_filter: ColumnElement) -> Select:
        return (
    ...
    def get_data_for_dependency_graph(self, uid: str) -> list[DepGraphData]:
        ...
        with self.get_read_only_session() as session:
            ...
            vfp_dict = self.get_vfps_in_parent(uid)
            query = (
                select(
                    FileObjectEntry.uid,
                    FileObjectEntry.file_name,
