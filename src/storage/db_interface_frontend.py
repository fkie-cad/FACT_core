...
class FrontEndDbInterface(MongoInterfaceCommon):
    ...
    def rest_get_firmware_uids(self, offset, limit, query=None, recursive=False, inverse=False):
        if recursive:
            return self.generic_search(search_dict=query, skip=offset, limit=limit, only_fo_parent_firmware=True, inverted=inverse)
