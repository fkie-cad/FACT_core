...
class DbInterfaceCommon(ReadOnlyDbInterface):
    ...
    def get_summary(self, fo: FileObject, selected_analysis: str, invert: bool = False) -> Summary | None:
        if selected_analysis not in fo.processed_analysis:
            ...
            return None
        if isinstance(fo, Firmware):
            summary = self.get_fw_summary_for_plugin(fo.uid, selected_analysis, invert)
            # merge with existing summary of FW
            for entry in fo.processed_analysis.get(selected_analysis, {}).get('summary', []):
                if invert:
                    summary.setdefault(fo.uid, []).append(entry)
                else:
                    summary.setdefault(entry, []).append(fo.uid)
            return summary
        return self.get_fo_summary_for_plugin(fo, selected_analysis, invert)

    def get_fw_summary_for_plugin(self, root_uid: str, plugin: str, invert: bool = False) -> Summary:
        return self.get_fw_summary(root_uid, plugin, invert).get(plugin, {})
    ...
    def get_fo_summary(
        self, fo: FileObject, analysis_filter: str | None = None, invert: bool = False
    ) -> dict[str, Summary]:
        if not fo.list_of_all_included_files:
            fo.list_of_all_included_files = self.get_list_of_all_included_files(fo)
        return self._get_summary_for_uid_list(fo.list_of_all_included_files, analysis_filter, invert)

    def get_fo_summary_for_plugin(self, fo: FileObject, plugin: str, invert: bool = False) -> Summary:
        return self.get_fo_summary(fo, plugin, invert).get(plugin, {})

    def get_fw_summary(self, uid: str, analysis_filter: str | None = None, invert: bool = False) -> dict[str, Summary]:
        """
        ...
        with self.get_read_only_session() as session:
            ...
            return self._collect_summary_result(query, session, invert)

    def _get_summary_for_uid_list(
        self, uid_list: set[str] | list[str], analysis_filter: str | None = None, invert: bool = False
    ) -> dict[str, Summary]:
        with self.get_read_only_session() as session:
    ...
    @staticmethod
    def _collect_summary_result(query: Select, session: Session, invert: bool) -> dict[str, Summary]:
        summary = {}
        for uid, summary_list, plugin in session.execute(query):  # type: str, list[str], str
            for item in set(summary_list or []):
                if invert:
                    summary.setdefault(plugin, {}).setdefault(uid, []).append(item)
                else:
