...
def main(skip_docker, radare, nginx, distribution):
    ...
    # The "unmet peer-dependencys" can be ignored according to [1].
    # Also we use `--ignore-engines` since we probably don't need any nodejs code and
    #
    # [1]: https://stackoverflow.com/questions/61586524/yarn-warning-bootstrap4-x-x-has-unmet-peer-dependency-jquery1-9-1-3
    run_cmd_with_logging('yarnpkg install --ignore-engines --cwd ../../src/web_interface/static')
