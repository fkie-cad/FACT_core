...
class CompareRoutes(ComponentBase):
    ...
    def _get_compare_plugin_views(self, compare_result):
        ...
        with suppress(KeyError):
            ...
            with get_shared_session(self.db.template()) as db:
                for plugin in used_plugins:
                    view = db.get_view(plugin)
                    if view:
    ...
    def start_compare(self):
        ...
        with get_shared_session(self.db.comparison()) as db:
            if not db.objects_exist(comparison_id):
                return render_template('compare/error.html', error='Not all UIDs found in the DB')

            if not redo and db.comparison_exists(comparison_id):
    ...
    def browse_comparisons(self):
        with get_shared_session(self.db.comparison()) as db:
            ...
            try:
                compare_list = db.page_comparison_results(skip=per_page * (page - 1), limit=per_page)
            except Exception as exception:
                error_message = f'Could not query database: {type(exception)}'
                logging.error(error_message, exc_info=True)
                return render_template('error.html', message=error_message)

            total = db.get_total_number_of_results()
