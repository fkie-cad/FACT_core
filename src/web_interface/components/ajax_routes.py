...
class AjaxRoutes(ComponentBase):
    ...
    def _generate_file_tree(self, root_uid: str, uid: str, whitelist: List[str]) -> FileTreeNode:
        ...
        with get_shared_session(self.db.frontend()) as db:
            child_uids = [
                child_uid
                for child_uid in db.get_object(uid).files_included
                if whitelist is None or child_uid in whitelist
            ]
            for node in db.generate_file_tree_nodes_for_uid_list(child_uids, root_uid, uid, whitelist):
    ...
    def ajax_get_summary(self, uid, selected_analysis):
        with get_shared_session(self.db.frontend()) as db:
            firmware = db.get_object(uid, analysis_filter=selected_analysis)
            summary_of_included_files = db.get_summary(firmware, selected_analysis)
