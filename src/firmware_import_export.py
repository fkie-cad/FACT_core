    ...
    def _import_file(self, path: Path):
        with ZipFile(path, 'r') as zip_file:
            if 'data.json' not in zip_file.namelist():
                logging.error('Error: data.json not found in uploaded import file')
                return
            try:
            ...
            if not all(k in data for k in ['db_revision', 'files', 'firmware', 'uid']):
                logging.error('Error: data.json is missing mandatory keys')
                return
            ...
            logging.info(f'Successfully imported {imported_files} files and {imported_objects} DB entries from {path}')
...
def _parse_args(args=None):
    ...
    parser_export.add_argument('UIDs', nargs='+', help='The UIDs of the firmware(s) to export')
