...
from test.acceptance.conftest import test_fw_a, test_fw_c, upload_test_firmware
...
class TestAcceptanceCompareFirmwares:
    ...
    def _start_comparison(self, test_client):
        rv = test_client.get('/compare', follow_redirects=True)
        assert b'Your comparison task is in progress.' in rv.data, 'comparison wait page not displayed correctly'
    ...
    def _show_comparison_browse(self, test_client):
        rv = test_client.get('/database/browse_comparisons')
        assert test_fw_a.name.encode() in rv.data, 'no comparison result shown in browse'

    def _show_analysis_without_comparison_list(self, test_client):
        rv = test_client.get(f'/analysis/{test_fw_a.uid}')
        assert b'Show list of known comparisons' not in rv.data

    def _show_analysis_with_comparison_list(self, test_client):
        rv = test_client.get(f'/analysis/{test_fw_a.uid}')
    ...
    def test_compare_firmwares(self, test_client, analysis_finished_event, comparison_finished_event):
        ...
        self._show_analysis_without_comparison_list(test_client)
        self._add_firmwares_to_compare(test_client)
        self._start_comparison(test_client)
        ...
        self._show_comparison_browse(test_client)
        self._show_analysis_with_comparison_list(test_client)
