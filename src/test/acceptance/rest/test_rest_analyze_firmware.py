...
class TestRestFirmware:
    ...
    def _rest_get_analysis_result(self, test_client):
        rv = test_client.get(f'/rest/firmware/{self.test_container_uid}', follow_redirects=True)
    ...
    def _rest_update_analysis_bad_analysis(self, test_client):
        ...
        rv = test_client.put(f'/rest/firmware/{self.test_container_uid}?update={query}', follow_redirects=True)
    ...
    def _rest_update_analysis_success(self, test_client):
        ...
        rv = test_client.put(f'/rest/firmware/{self.test_container_uid}?update={update}', follow_redirects=True)
    ...
    def test_run_from_upload_to_show_analysis_and_search(self, test_client, backend_services, analysis_finished_event, elements_finished_analyzing):
        self._rest_upload_firmware(test_client)
        analysis_finished_event.wait(timeout=15)
        elements_finished_analyzing.value = 4 * 2  # only one plugin to update, so we offset with 4 times 2 plugins
