...
def test_get_summary(backend_db, common_db):
    ...
    result_sum = common_db.get_summary(fw, 'dummy')
...
def test_collect_summary(backend_db, common_db):
    fo, fw = create_fw_with_child_fo()
    backend_db.insert_object(fw)
    backend_db.insert_object(fo)
    fo_list = [fo.uid]
    result_sum = common_db._collect_summary(fo_list, 'dummy')
...
def test_get_summary_of_one_error_handling(backend_db, common_db):
    fo, fw = create_fw_with_child_fo()
    backend_db.insert_object(fw)
    backend_db.insert_object(fo)

    result_sum = common_db._get_summary_of_one(None, 'foo')
    assert result_sum == {}, 'None object should result in empty dict'
    result_sum = common_db._get_summary_of_one(fw, 'non_existing_analysis')
    assert result_sum == {}, 'analysis non-existent should lead to empty dict'


def test_update_summary(common_db):
    orig = {'a': ['a']}
    update = {'a': ['aa'], 'b': ['aa']}
    common_db._update_summary(orig, update)
    assert 'a' in orig
    assert 'b' in orig
    assert 'a' in orig['a']
    assert 'aa' in orig['a']
