...
def test_get_device_name_dict(db):
    ...
    assert db.frontend.get_device_name_dict() == {
        'class1': {
            'vendor1': ['name1', 'name2'], 'vendor2': ['name1']
        },
        'class2': {
            'vendor1': ['name1']
        },
...
def test_generic_search_or(db):
    ...
    assert set(db.frontend.generic_search({'$or': {
        'file_name': 'other_file.zip', 'size': {
            '$lt': 20
        }
    }})) == {'uid_1', 'uid_2'}
    # "$or" query should still match if there is a firmware attribute in the query
    assert set(db.frontend.generic_search({'$or': {
        'file_name': 'some_file.zip', 'vendor': 'some_vendor'
    }})) == {'uid_1'}
...
def test_generic_search_parent(db):
    ...
    assert db.frontend.generic_search({'processed_analysis.plugin.foo': 'bar'},
                                      only_fo_parent_firmware=True) == [fw.uid]
...
def test_generic_search_nested(db):
    ...
    assert db.frontend.generic_search({'processed_analysis.plugin.nested.key': {
        '$in': ['value', 'other_value']
    }}) == [fo.uid]
...
def test_generic_search_json_types(db):
    fo, fw = create_fw_with_child_fo()
    fo.processed_analysis = {
        'plugin': generate_analysis_entry(analysis_result={
            'str': 'a', 'int': 1, 'float': 1.23, 'bool': True
        })
    }
    db.backend.insert_object(fw)
    db.backend.insert_object(fo)
...
def test_inverted_search(db):
    ...
    assert db.frontend.generic_search({'file_name': 'foo.bar'}, only_fo_parent_firmware=True) == [fw.uid]
    assert db.frontend.generic_search({'file_name': 'foo.bar'}, only_fo_parent_firmware=True,
                                      inverted=True) == ['some_other_fw']
...
def test_rest_get_firmware_uids(db):
    ...
    assert sorted(db.frontend.rest_get_firmware_uids(offset=None,
                                                     limit=None)) == [parent_fw.uid, test_fw1.uid, test_fw2.uid]
    assert sorted(db.frontend.rest_get_firmware_uids(query={}, offset=0,
                                                     limit=0)) == [parent_fw.uid, test_fw1.uid, test_fw2.uid]
    ...
    assert sorted(db.frontend.rest_get_firmware_uids(offset=None, limit=None,
                                                     query={'vendor': 'foo_vendor'})) == [test_fw1.uid, test_fw2.uid]
