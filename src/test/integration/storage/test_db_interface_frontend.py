...
class TestStorageDbInterfaceFrontend(TestBase):
    ...
    def test_get_meta_list(self):
        ...
        assert test_output[1] == 'test_vendor test_router - 0.1 (Router)', 'Firmware not successfully received'
        assert isinstance(test_output[2], dict), 'tag field is not a dict'
    ...
    def test_get_meta_list_of_fo(self):
        ...
        assert meta_list[0][0] == test_fo.uid, 'uid of object not correct'
    ...
    def test_get_firmware_attribute_list(self):
        ...
        assert self.db_frontend_interface.get_firmware_attribute_list('device_name', {'vendor': 'test_vendor', 'device_class': 'Router'}) == ['test_router']
    ...
    def test_get_data_for_nice_list(self):
        ...
        assert sorted(['size', 'current_virtual_path', 'uid', 'mime-type', 'files_included']) == sorted(nice_list_data[0].keys())
        assert nice_list_data[0]['uid'] == self.test_firmware.uid

    def test_generic_search(self):
        self.db_backend_interface.add_firmware(self.test_firmware)
        # str input
        result = self.db_frontend_interface.generic_search('{"file_name": "test.zip"}')
        assert result == [self.test_firmware.uid], 'Firmware not successfully received'
        # dict input
        result = self.db_frontend_interface.generic_search({'file_name': 'test.zip'})
        assert result == [self.test_firmware.uid], 'Firmware not successfully received'
    ...
    def test_all_uids_found_in_database(self):
        self.db_backend_interface.client.drop_database(self.config.get('data_storage', 'main_database'))
        uid_list = [self.test_firmware.uid]
        assert not self.db_frontend_interface.all_uids_found_in_database(uid_list)
        self.db_backend_interface.add_firmware(self.test_firmware)
        assert self.db_frontend_interface.all_uids_found_in_database([self.test_firmware.uid])
    ...
    def test_get_x_last_added_firmwares(self):
        ...
        assert result[0][0], test_fw_three.uid == 'last firmware is not first entry'
        assert result[1][0], test_fw_two.uid == 'second last firmware is not the second entry'
    ...
    def test_get_other_versions_of_firmware(self):
        ...
        assert {'_id': parent_fw2.uid, 'version': '2'} in other_versions
        assert {'_id': parent_fw3.uid, 'version': '3'} in other_versions
