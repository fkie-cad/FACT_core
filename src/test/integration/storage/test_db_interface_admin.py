...
def test_delete_fo(db, admin_db):
...
def test_delete_cascade(db, admin_db):
...
def test_remove_vp_no_other_fw(db, admin_db):
    fo, fw = create_fw_with_child_fo()
    db.backend.insert_object(fw)
    db.backend.insert_object(fo)

    with admin_db.get_read_write_session() as session:
        removed_vps, deleted_files = admin_db._remove_virtual_path_entries(fw.uid, fo.uid, session)  # pylint: disable=protected-access

    assert removed_vps == 0
    assert deleted_files == 1
    assert admin_db.intercom.deleted_files == [fo.uid]


def test_remove_vp_other_fw(db, admin_db):
    fo, fw = create_fw_with_child_fo()
...
def test_delete_but_fo_is_in_fw(db, admin_db):
    fo, fw = create_fw_with_child_fo()
