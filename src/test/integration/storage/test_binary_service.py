...
@pytest.fixture
def binary_service(backend_db, cfg_tuple):
    cfg, _ = cfg_tuple
    _init_test_data(cfg.data_storage.firmware_file_storage_directory, backend_db)
    yield BinaryService()


# TODO this could be a marker:
# pytest.mark.backend_db_add_objects(fos: List[FileObject])
# Probably a marker is not that good for this. The name is very long.
