...
def test_add_and_get_comparison_result(comparison_db, backend_db):
    fw_one, _, _, comparison_id = _add_comparison(comparison_db, backend_db)
    retrieved = comparison_db.get_comparison_result(comparison_id)
...
def test_get_not_existing_result(comparison_db, backend_db):
    fw_one, fw_two, _, comparison_id = _create_comparison()
...
def test_calculate_comparison_id(comparison_db):
    _, _, comparison_dict, comparison_id = _create_comparison()
    comp_id = comparison_db._calculate_comp_id(comparison_dict)
    assert comp_id == comparison_id


def test_comp_id_incomplete_entries(comparison_db):
    comparison_dict = {'general': {'stat_1': {'a': None}, 'stat_2': {'b': None}}}
    comp_id = comparison_db._calculate_comp_id(comparison_dict)
    assert comp_id == 'a;b'
...
def test_get_latest_removed_firmware(comparison_db, backend_db, admin_db):
    fw_one, fw_two, comparison_dict, _ = _create_comparison()
    backend_db.insert_multiple_objects(fw_one, fw_two)
    comparison_db.add_comparison_result(comparison_dict)

    result = comparison_db.page_comparison_results(limit=10)
    assert result != [], 'A comparison result should be available'

    admin_db.delete_firmware(fw_two.uid)

    result = comparison_db.page_comparison_results(limit=10)

    assert result == [], 'No comparison result should be available'
...
def test_get_total_number_of_results(comparison_db, backend_db):
    ...
    assert number == 1, 'no comparison result found in database'
...
def test_get_exclusive_files(comparison_db, backend_db, root_uid, expected_result):
    fw_one, fw_two, comparison_dict, comp_id = _create_comparison()
    comparison_dict['plugins'] = {'File_Coverage': {'exclusive_files': {'the_root_uid': ['uid1', 'uid2']}}}
    backend_db.insert_multiple_objects(fw_one, fw_two)
    comparison_db.add_comparison_result(comparison_dict)
...
def _create_comparison(uid1='uid1', uid2='uid2'):
    ...
    comparison_id = f'{fw_one.uid};{fw_two.uid}'
    return fw_one, fw_two, comparison_dict, comparison_id


def _add_comparison(comparison_db, backend_db, uid1='uid1', uid2='uid2'):
    fw_one, fw_two, comparison_dict, comparison_id = _create_comparison(uid1=uid1, uid2=uid2)
    backend_db.insert_multiple_objects(fw_one, fw_two)
    comparison_db.add_comparison_result(comparison_dict)
    return fw_one, fw_two, comparison_dict, comparison_id
