...
def test_comparison_exists(real_database, comp_db):
    comp_id = 'uid1;uid2'
    assert comp_db.comparison_exists(comp_id) is False
    _add_comparison(comp_db, real_database)
    assert comp_db.comparison_exists(comp_id) is True


def test_add_and_get_comparison_result(real_database, comp_db):
    fw_one, _, _, compare_id = _add_comparison(comp_db, real_database)
...
def test_get_not_existing_result(real_database, comp_db):
    fw_one, fw_two, _, compare_id = _create_comparison()
    real_database.backend.add_object(fw_one)
    real_database.backend.add_object(fw_two)
    result = comp_db.get_comparison_result(compare_id)
    assert result is None


def test_calculate_comparison_id(real_database, comp_db):  # pylint: disable=unused-argument
    _, _, compare_dict, compare_id = _create_comparison()
    comp_id = comp_db._calculate_comp_id(compare_dict)
    assert comp_id == compare_id


def test_comp_id_incomplete_entries(real_database, comp_db):  # pylint: disable=unused-argument
    compare_dict = {'general': {'stat_1': {'a': None}, 'stat_2': {'b': None}}}
    comp_id = comp_db._calculate_comp_id(compare_dict)
    assert comp_id == 'a;b'


def test_get_latest_comparisons(real_database, comp_db):
    before = time()
    fw_one, fw_two, _, _ = _add_comparison(comp_db, real_database)
...
def test_delete_fw_cascades_to_comp(real_database, comp_db):
    _, fw_two, _, comp_id = _add_comparison(comp_db, real_database)

    with comp_db.get_read_only_session() as session:
        assert session.get(ComparisonEntry, comp_id) is not None

    real_database.admin.delete_firmware(fw_two.uid)

    with comp_db.get_read_only_session() as session:
        assert session.get(ComparisonEntry, comp_id) is None, 'deletion should be cascaded if one FW is deleted'


def test_get_latest_removed_firmware(real_database, comp_db):
...
def test_get_total_number_of_results(real_database, comp_db):
    _add_comparison(comp_db, real_database)
...
def test_get_exclusive_files(real_database, comp_db, root_uid, expected_result):
...
def _add_comparison(comp_db, real_database, uid1='uid1', uid2='uid2'):
