...
# TODO add to DatabaseInterfaces
@pytest.fixture
def stats_viewer():
...
def test_update_stats(stats_update_db):  # pylint: disable=unused-argument
...
def test_get_stats(stats_update_db, stats_viewer):  # pylint: disable=unused-argument
...
def test_get_stats_list(stats_update_db, stats_viewer):  # pylint: disable=unused-argument
    stats_update_db.update_statistic('foo', {'foo': 'bar'})
...
def test_count_distinct_values(backend_db, stats_update_db):
    ...
    assert stats_update_db.count_distinct_values(FirmwareEntry.vendor) == [('vendor_2', 1), ('vendor_1', 2)], 'sorted wrongly'
...
def test_count_distinct_array(backend_db, stats_update_db, q_filter, plugin, expected_result):
    insert_test_fw(backend_db, 'root_fw', vendor='foobar')
    insert_test_fo(
        backend_db, 'fo1', parent_fw='root_fw', analysis={'foo': generate_analysis_entry(analysis_result={'key': ['value1']})}
    )
    insert_test_fo(
        backend_db,
        'fo2',
    ...
    stats = stats_update_db.count_distinct_values_in_array(AnalysisEntry.result['key'], plugin=plugin, q_filter=q_filter)
