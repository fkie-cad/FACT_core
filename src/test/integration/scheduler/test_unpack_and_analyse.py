...
class TestFileAddition:
    ...
    def setup(self):
        ...
        self.enter_patch = patch(target='helperFunctions.database.ConnectTo.__enter__', new=lambda _: self.mocked_interface)
        self.enter_patch.start()
        self.exit_patch = patch(target='helperFunctions.database.ConnectTo.__exit__', new=fake_exit)
    ...
    def test_unpack_and_analyse(self):
        ...
        assert len(processed_container.processed_analysis) >= 3, 'at least one analysis not done'
