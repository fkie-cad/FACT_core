...
def test_app_re_analyze_get_invalid_firmware(test_client):
    rv = test_client.get('/update-analysis/invalid')
    assert b'File not found in database: invalid' in rv.data


def test_app_re_analyze_get_valid_firmware(test_client):
...
def test_app_re_analyze_post_valid(web_frontend, test_client):
    form_data = {
        'device_name': '',
    ...
    assert web_frontend.intercom.tasks[0].uid == TEST_FW.uid, 'fw not added to intercom'
    assert 'new_system' in web_frontend.intercom.tasks[0].scheduled_analysis, 'new analysis system not scheduled'
