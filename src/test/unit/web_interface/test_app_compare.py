...
def test_start_compare(web_frontend, test_client):
    with test_client as tc:
        with tc.session_transaction() as test_session:
        ...
        assert len(web_frontend.intercom.tasks) == 1, 'task not added'
        assert web_frontend.intercom.tasks[0] == ('uid1;uid2', None), 'task not correct'


def test_start_compare__force(web_frontend, test_client):
    with test_client as tc:
        with tc.session_transaction() as test_session:
            test_session['uids_for_comparison'] = {'uid1': None, 'uid2': None}
        ...
        assert b'Your compare task is in progress' in rv.data
        assert len(web_frontend.intercom.tasks) == 1, 'task not added'
        assert web_frontend.intercom.tasks[0] == ('uid1;uid2', True), 'task not correct'
