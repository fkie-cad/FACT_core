...
class TestScheduleInitialAnalysis(AnalysisSchedulerTest):
    ...
    def test_whole_run_analysis_selected(self):
        ...
        assert len(test_fw.processed_analysis) == 3, 'analysis not done'
        assert test_fw.processed_analysis['dummy_plugin_for_testing_only']['1'] == 'first result', 'result not correct'
        assert test_fw.processed_analysis['dummy_plugin_for_testing_only']['summary'] == ['first result', 'second result']
        assert 'file_hashes' in test_fw.processed_analysis.keys(), 'Mandatory plug-in not executed'
        assert 'file_type' in test_fw.processed_analysis.keys(), 'Mandatory plug-in not executed'
...
def mock_load_plugins(self):
    self.analysis_plugins = {
        'dummy_plugin_for_testing_only': PluginMock(whitelist='foo, bar'),
        'file_type': PluginMock(), 'file_hashes': PluginMock()
    }


class TestScheduleInitialAnalysisNoPlugins(AnalysisSchedulerTest):
    def setup(self):
        self.load_plugins_patch = mock.patch(target='scheduler.Analysis.AnalysisScheduler.load_plugins', new=mock_load_plugins)
        self.load_plugins_patch.start()
        super().setup()

    def teardown(self):
        self.load_plugins_patch.stop()
        super().teardown()
...
class TestAnalysisSchedulerBlacklist:
    ...
    def test_get_blacklist_and_whitelist_from_plugin(self):
        self.sched.analysis_plugins['test_plugin'] = PluginMock(['foo'], ['bar'])
    ...
    def test_get_blacklist_and_whitelist_from_plugin__missing_in_plugin(self):
        self.sched.analysis_plugins['test_plugin'] = PluginMock(['foo'])
        blacklist, whitelist = self.sched._get_blacklist_and_whitelist_from_plugin('test_plugin')
        assert whitelist == []
    ...
    def test_next_analysis_is_blacklisted__blacklisted(self):
        self.sched.analysis_plugins[self.test_plugin] = PluginMock(blacklist=['blacklisted_type'])
    ...
    def test_next_analysis_is_blacklisted__not_blacklisted(self):
        self.sched.analysis_plugins[self.test_plugin] = PluginMock(blacklist=[])
    ...
    def test_next_analysis_is_blacklisted__whitelisted(self):
        self.sched.analysis_plugins[self.test_plugin] = PluginMock(whitelist=['whitelisted_type'])
    ...
    def test_next_analysis_is_blacklisted__not_whitelisted(self):
        self.sched.analysis_plugins[self.test_plugin] = PluginMock(whitelist=['some_other_type'])
    ...
    def test_next_analysis_is_blacklisted__whitelist_precedes_blacklist(self):
        self.sched.analysis_plugins[self.test_plugin] = PluginMock(blacklist=['test_type'], whitelist=['test_type'])
        self.file_object.processed_analysis['file_type']['mime'] = 'test_type'
        blacklisted = self.sched._next_analysis_is_blacklisted(self.test_plugin, self.file_object)
        assert blacklisted is False

        self.sched.analysis_plugins[self.test_plugin] = PluginMock(blacklist=[], whitelist=['some_other_type'])
        self.file_object.processed_analysis['file_type']['mime'] = 'test_type'
        blacklisted = self.sched._next_analysis_is_blacklisted(self.test_plugin, self.file_object)
        assert blacklisted is True
