...
class TestVirtualFilePath(unittest.TestCase):
    ...
    def test_add_file_object(self):
        ...
        self.assertEqual(root.virtual_file_path[root.get_uid()], [root.get_uid()], 'virtual file path of root file not correct')
        ...
        self.assertIn(root.get_uid(), child_one.virtual_file_path.keys(), 'no virtual file path for root available')
        self.assertEqual(child_one.virtual_file_path[root.get_uid()][0], '{}|{}'.format(root.get_uid(), child_one.file_path), 'virtual file path not correct')
        self.assertEqual(child_of_child_one.virtual_file_path[root.get_uid()][0], '{}|{}|{}'.format(root.get_uid(), child_one.get_uid(), child_of_child_one.file_path))
    ...
    def test_add_file_object_path_already_present(self):
        ...
        self.assertEqual(len(child.virtual_file_path[root.get_uid()]), 1, 'number of pathes should be one')
