...
from helperFunctions.yara import (
    ...
    StringMatch,
    compile_rules,
    compile_signatures_of_plugin,
    get_all_matched_strings,
    is_valid_yara_source_file,
    scan_dir,
    scan_file,
    scan_files,
)
...
def test_compile_rules(temp_compiled_rules_from_dir):
    assert temp_compiled_rules_from_dir.exists()
    assert temp_compiled_rules_from_dir.stat().st_size > 0
    with temp_compiled_rules_from_dir.open('rb') as fp:
        assert fp.read(4) == b'YARA', 'a compiled rule file should start with the magic "YARA"'
...
def test_compile_plugin_yara_signatures(tmp_path):
    ...
    compile_signatures_of_plugin(plugin_dir, output_dir)
...
def test_is_valid_yara_source_file(temp_compiled_rules):
    assert is_valid_yara_source_file(TEST_DIR / 'test.yara') is True
    assert is_valid_yara_source_file(temp_compiled_rules) is False
    assert is_valid_yara_source_file(Path(__file__)) is False
