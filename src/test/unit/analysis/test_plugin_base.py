...
class TestPluginBaseCore:
    def test_object_processing_no_children(self, analysis_plugin):
        ...
        assert processed_object.processed_analysis['dummy_plugin_for_testing_only']['plugin_version'] == '0.0', 'plugin version missing in results'
        assert processed_object.processed_analysis['dummy_plugin_for_testing_only']['analysis_date'] > 1, 'analysis date missing in results'
...
class TestPluginNotRunning:
    # TODO use analysis_plugin fixture
    ...
    def multithread_config_test(self, multithread_flag, threads_in_config, threads_wanted):
        self.p_base = DummyPlugin(self.MockPluginAdministrator(), no_multithread=multithread_flag)
    ...
    def test_init_result_dict(self):
        self.p_base = DummyPlugin(self.MockPluginAdministrator())
...
@pytest.mark.AnalysisPluginClass.with_args(DummyPlugin)
@pytest.mark.plugin_start_worker
@pytest.mark.skip('TODO somehow setting analysis_plugin.TIMEOUT has no effect on AnalysisBasePlugin.TIMEOUT')
def test_timeout(analysis_plugin):
...
def test_attribute_check():
    ...
    with pytest.raises(PluginInitException):
        AnalysisBasePlugin(MockPluginAdministrator())
