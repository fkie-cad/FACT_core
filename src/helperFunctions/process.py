...
def check_worker_exceptions(
    ...
    children will be terminated. If ``restart_workers`` is set to `False`, the worker will be restarted with the
    ``worker_function`` as entrypoint.
    Otherwise, the worker will not be restarted. In this case, the function will always return ``False``. If
    ``restart_workers`` is set to `True` and an exception occurs, the worker will not be restarted and the return value
    is ``True``.
    ...
    :return: ``True`` if an exception occurred in any process and ``restart_workers`` set to `True`. Returns ``False``
        otherwise.
    '''
    return_value = False
