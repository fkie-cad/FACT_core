...
def test_hex_preview_merge():
    ...
    assert highlighted.count('<span') == 4, 'highlight zones should be merged'  # noqa: PLR2004
...
def test_hex_preview_data_rows():
    ...
    assert highlighted.count('\n') == 4  # 2 header lines and 3 rows (len(input) // 4)  # noqa: PLR2004
    assert len(highlighted.split('\n')[-1].split('|')[2]) == 14, 'partial rows should be filled up'  # noqa: PLR2004
